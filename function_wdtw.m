...
%Compute Weighted Dynamic Time Warping distance between two time series X and Y with Sakoe-Chiba Band 
%-------------------------------------------------------------------------------------------
function [size_warping_path, path, dtw_distance] = function_wdtw(X, Y, weight_meanCluster, sakoe_chiba_band)
global alpha;
...
for i = 1 : n
	for j = 1 : m
		C(i,j) =  ( 1 / (weight_meanCluster(j) .^ alpha) ) * ( ( X(i) - Y(j) ) ^ 2 ); %(1/WY(j)*(X(i)-Y(j))^2)
	end;
end;
...
%Compute weighted Dynamic Time Warping Distance
%-------------------------------------------------------------------------------------------
